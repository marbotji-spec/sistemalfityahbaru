    ...
    <main className="min-h-screen bg-[#f7fafc]">
      <header className="border-b border-[#d8e3ec] bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-10">
          ...
          <button className="rounded-lg bg-[#e52335] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#c91d2e]">Masuk sistem</button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 lg:px-10 lg:py-10">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.25fr_0.75fr]">
          <section id="aktivitas" className="rounded-xl border border-[#d8e3ec] bg-white shadow-[0_2px_8px_rgba(17,43,69,0.04)]"><div className="flex items-center justify-between border-b border-[#d8e3ec] px-5 py-4"><div><h2 className="font-bold text-[#112b45]">Aktivitas mendatang</h2><p className="mt-1 text-sm text-slate-500">Jadwal pembelajaran dan setoran</p></div><button className="text-sm font-semibold text-[#147fbd] hover:text-[#e52335]">Lihat semua</button></div><div className="divide-y divide-[#d8e3ec]">{activities.map(([time, title, group, count]) => <div key={title} className="flex items-center gap-4 px-5 py-4"><div className="h-11 w-1 rounded-full bg-[#e52335]" /><div className="min-w-0 flex-1"><p className="text-xs font-medium text-[#1687c9]">{time}</p><p className="mt-1 font-semibold text-[#112b45]">{title}</p><p className="text-sm text-slate-500">{group}</p></div><span className="text-sm font-medium text-slate-500">{count}</span></div>)}</div></section>

          <section id="program" className="rounded-xl border border-[#d8e3ec] bg-[#112b45] p-6 text-white shadow-[0_2px_8px_rgba(17,43,69,0.08)]"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#5fc0f2]">Program utama</p><h2 className="mt-3 text-2xl font-bold">Tumbuh bersama Al-Qur&apos;an</h2><p className="mt-3 text-sm leading-6 text-slate-300">Kelola perkembangan TPA dan Tahfiz dengan catatan yang rapi, terukur, dan mudah dipantau.</p><div className="mt-7 space-y-3">{["TPA Al-Fityah", "Tahfiz Al-Qur'an"].map((program) => <div key={program} className="flex items-center justify-between rounded-lg bg-white/10 px-4 py-3 text-sm"><span>{program}</span><span className="text-[#5fc0f2]">&rarr;</span></div>)}</div></section>
        </div>

        <section className="mt-8 rounded-xl border border-[#d8e3ec] bg-white p-6 shadow-[0_2px_8px_rgba(17,43,69,0.04)]"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 className="font-bold text-[#112b45]">Aksi cepat</h2><p className="mt-1 text-sm text-slate-500">Mulai pekerjaan administrasi hari ini</p></div><div className="grid gap-3 sm:grid-cols-3"><button className="rounded-lg bg-[#e52335] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#c91d2e]">Tambah santri</button><button className="rounded-lg border border-[#1687c9] px-4 py-2.5 text-sm font-semibold text-[#147fbd] hover:bg-[#edf8fe]">Input absensi</button><button className="rounded-lg border border-[#1687c9] px-4 py-2.5 text-sm font-semibold text-[#147fbd] hover:bg-[#edf8fe]">Catat hafalan</button></div></div></section>
      </div>
    </main>
  );
}
